...
export const dartHandler: Handler = {
  ...
  async validate(_sourcePath) {
    try {
      ...
      return {
        valid: false,
        errors: ["Dart SDK not found. Install from https://dart.dev/get-dart"],
      };
    }
  },
};
...
function extractWithDartdoc(entryPoints: string[]): ASTModule[] {
  ...
  for (const entry of entryPoints) {
    ...
    execSync(cmd, {
      encoding: "utf-8",
      cwd: existsSync(path.join(resolvedEntry, "pubspec.yaml")) ? resolvedEntry : path.dirname(resolvedEntry),
  ...
  function convertDartdocElements(elements: DartdocElement[]): ASTModule[] {
    ...
    for (const [libName, libElements] of libraryMap) {
      ...
      for (const cls of classes) {
        ...
        const members = libElements.filter((e) => e.enclosingElement?.name === cls.name);
