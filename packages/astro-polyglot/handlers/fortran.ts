...
export const fortranHandler: Handler = {
  ...
  async generate(options) {
    ...
    const xmlDir = await runDoxygen({ inputDir: opts.sourcePath, filePatterns: ["*.f", "*.f90", "*.f95", "*.f03"], projectName: "Fortran" });
    ...
    return transformToMDX(modules, { outputDir: opts.output, language: "fortran", ...(opts.pagination !== undefined ? { pagination: opts.pagination } : {}) });
  },
  async validate() {
