...
interface JavaScriptHandlerOptions extends BaseHandlerOptions {
  entryPoints: string[];
  tsconfig?: string;
}

export const javascriptHandler: Handler = {
  name: "javascript",

  async generate(options) {
    const opts = options as unknown as JavaScriptHandlerOptions;
    const entryPoints = opts.entryPoints;

    if (!entryPoints || entryPoints.length === 0) {
      throw new Error("JavaScript handler requires at least one entryPoint");
    }

    const modules = await extractWithTypeDoc(entryPoints, opts.tsconfig);

    if (modules.length === 0) {
      throw new Error("TypeDoc extraction produced no modules for JavaScript");
    }
