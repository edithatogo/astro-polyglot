...
export const lexHandler: Handler = {
  ...
  async generate(options) {
    ...
    const xmlDir = await runDoxygen({ inputDir: opts.sourcePath, filePatterns: ["*.l", "*.ll", "*.flex"], projectName: "Lex/Flex" });
    ...
    return transformToMDX(modules, { outputDir: opts.output, language: "lex", ...(opts.pagination !== undefined ? { pagination: opts.pagination } : {}) });
  },
  async validate() {
