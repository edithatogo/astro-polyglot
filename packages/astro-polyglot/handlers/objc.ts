...
export const objcHandler: Handler = {
  ...
  async generate(options) {
    ...
    const xmlDir = await runDoxygen({ inputDir: opts.sourcePath, filePatterns: ["*.m", "*.mm", "*.h"], projectName: "Objective-C" });
    ...
    return transformToMDX(modules, { outputDir: opts.output, language: "objc", ...(opts.pagination !== undefined ? { pagination: opts.pagination } : {}) });
  },
  async validate() {
