...
export const scalaHandler: Handler = {
  ...
  async validate(_sourcePath) {
    try {
      ...
      return {
        valid: false,
        errors: ["scaladoc not found. Install the Scala toolchain from https://www.scala-lang.org/download/"],
      };
    }
  },
};
...
function extractWithScaladoc(entryPoints: string[], classpath?: string): ASTModule[] {
  ...
  if (!existsSync(tmpDir)) {
    throw new Error("scaladoc did not produce output. Ensure scaladoc is installed and entry points are valid.");
  }
...
function parseScaladocDir(dir: string): ASTModule[] {
   ...
   */
  function extractParamDescription(comment?: ScaladocDocument["comment"], paramName?: string): string | undefined {
    if (!comment?.tags || !paramName) return undefined;
    const tag = comment.tags.find((t) => t.tag === "@param" && t.paramName === paramName);
