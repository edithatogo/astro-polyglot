...
interface VbNetOptions extends BaseHandlerOptions {
  projectPath: string;
}

export const vbnetHandler: Handler = {
  name: "vbnet",
  async generate(options) {
    const opts = options as unknown as VbNetOptions;
    if (!opts.projectPath) throw new Error("VB.NET handler requires a projectPath option");
    ...
    if (!xmlDocPath) throw new Error("Could not find XML documentation file for VB.NET project");
