...
async function extractWithTypeDoc(entryPoints: string[], tsconfig?: string): Promise<ASTModule[]> {
  ...
  const { Application, TSConfigReader } = typedoc;
...
async function main() {
  ...
  if (entryPoints.length === 0) {
    const errOutput: Output = {
      errors: [{ entry_point: '', error: 'No --entry-points provided. Usage: npx tsx scripts/typescript_extract.ts --entry-points src/index.ts' }],
    };
