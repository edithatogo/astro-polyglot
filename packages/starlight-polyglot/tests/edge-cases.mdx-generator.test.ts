...
import { type ASTClass, type ASTFunction, type ASTModule, transformToMDX } from "../core/mdx-generator";
...
describe("special characters in names", () => {
  it("handles dollar sign in module name", () => {
    const modules: ASTModule[] = [{ name: "$special", docstring: "Dollar module." }];
    const output = transformToMDX(modules, { outputDir: "api/py" });
  ...
  it("handles at-sign in module name", () => {
    const modules: ASTModule[] = [{ name: "@decorators", docstring: "At module." }];
    const output = transformToMDX(modules, { outputDir: "api/py" });
    expect(output.pages[0]!.path).not.toContain("@");
  });

  it("handles hash in module name", () => {
    const modules: ASTModule[] = [{ name: "#hashtag", docstring: "Hash module." }];
    const output = transformToMDX(modules, { outputDir: "api/py" });
    expect(output.pages[0]!.path).not.toContain("#");
  });

  it("handles spaces in module name", () => {
    const modules: ASTModule[] = [{ name: "my module name", docstring: "Spaced." }];
    const output = transformToMDX(modules, { outputDir: "api/py" });
    expect(output.pages[0]!.path).not.toMatch(/\s/);
  });

  it("handles Japanese characters in module name", () => {
    const modules: ASTModule[] = [{ name: "モジュール", docstring: "Japanese module." }];
    const output = transformToMDX(modules, { outputDir: "api/py" });
